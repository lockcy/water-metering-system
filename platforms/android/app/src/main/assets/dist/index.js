...
__vue_options__.__file = "D:\\weex\\water-metering-system\\src\\index.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
...
exports.default = {
	...
	created: function created() {
		...
		storage.removeItem('objectId', function (res) {
			if (res.result == 'success') {}
		});
		storage.removeItem('username', function (res) {
			if (res.result == 'success') {}
		});
		storage.removeItem('uid', function (res) {
			if (res.result == 'success') {}
		});
		...
		handleSubmit: function handleSubmit() {
			...
			if (this.username == '' || this.password == '') {
				...
				stream.fetch({
					...
					if (res.status == 200) {
						//toast('登陆成功');
						_this.objectId = res.data.objectId;
						storage.setItem('objectId', _this.objectId, function (res) {
							if (res.result == 'success') {}
						});
						storage.setItem('username', _this.username, function (res) {
							if (res.result == 'success') {
								(0, _router2.default)('main');
							}
						});
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["tab-holder"]
  }, [_c('div', {
    staticClass: ["register"],
    on: {
      "click": _vm.register
    }
  }, [_c('text', [_vm._v("注册用户")])]), _c('div', {
    staticClass: ["forget"],
    on: {
      "click": _vm.forget
    }
